...
async def start(message: types.Message):
    ...
    await message.reply(errors.ALREADY_REGISTERED, reply_markup=keyboards.StartButtons1.keyboard, reply=False)


@dp.message_handler(Timeout(), commands='cancel')
async def cancel(message: types.Message):
    user_status.pop(message.from_user.id, None)
    await message.reply('Вы остановили выполнение операции', reply=False)


@dp.message_handler(commands="mi")
async def timetable_mi(message: types.Message):
    await message.reply('Выберите день недели', reply_markup=keyboards.all_command_buttons('allmi'), reply=False)
...
async def help_command(message: types.Message):
    ...
    await message.reply(errors.SHOULD_REGISTER, reply_markup=keyboards.StartButton.keyboard, reply=False)
...
async def lesson_status(message: types.Message):
    ...
    if not BotDB.user_exists(message.from_user.id):
        return await message.reply(errors.SHOULD_REGISTER, reply=False)
    if datetime.today().weekday() == 6:
        return await message.reply(errors.NO_LESSONS, reply=False)
    ...
    if current_time <= 915:
        for k, start_lesson in enumerate((540, 580, 590, 630, 645, 685, 700, 740, 755, 795, 815, 855, 875, 915), start=1):
            if 915 >= current_time < start_lesson:
                ...
                break
    await message.reply(e, reply_markup=keyboards.CallScheduleButton.keyboard, reply=False)


@dp.message_handler(Timeout(), commands=["today", "t"])
async def today(message: types.Message):
    if BotDB.user_exists(message.from_user.id):
        return await message.reply(Json.timetable(message.from_user.id, 0), reply=False)
    await message.reply(errors.SHOULD_REGISTER, reply=False)


@dp.message_handler(Timeout(), commands=["next", "n"])
async def next_day(message: types.Message):
    if BotDB.user_exists(message.from_user.id):
        return await message.reply(Json.timetable(message.from_user.id, -1), reply=False)
    await message.reply(errors.SHOULD_REGISTER, reply=False)
...
async def all_days(message: types.Message):
    if not BotDB.user_exists(message.from_user.id):
        return await message.reply(errors.SHOULD_REGISTER, reply=False)
    await message.reply('Выберите день недели', reply_markup=keyboards.all_command_buttons(), reply=False)
...
async def free_auditories(message: types.Message):
    await message.reply('Выберите день недели', reply_markup=keyboards.all_command_buttons('allfr'), reply=False)
...
async def thcom(message: types.Message):
    if BotDB.user_exists(message.from_user.id):
        ...
        return await message.reply('Выберите класс', reply_markup=keyboards.get_forms_keyboard(), reply=False)
    await message.reply(errors.SHOULD_REGISTER, reply_markup=keyboards.StartButton.keyboard, reply=False)
